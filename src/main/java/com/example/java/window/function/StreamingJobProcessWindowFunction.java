package com.example.java.window.function;
...
public class StreamingJobProcessWindowFunction {
    ...
    public static void main(String[] args) throws Exception {
        ...
        streamEnv
                ...
                }, new ProcessWindowFunction<Order, OrderSummary, Tuple, TimeWindow>() {
                    @Override
                    public void process(Tuple tuple, Context context, Iterable<Order> elements, Collector<OrderSummary> out) throws Exception {
                        elements.forEach(element -> out.collect(
                                new OrderSummary(context.window().getStart(), context.window().getEnd(),
                                        element.userId, 1, element.amount, element.total)));
                    }
                }).print();
